...
- **G1 — Closest internal prior (RES.009 body §1 + notebook T2) — CORRECTED (direct-audit finding):** RES.009 §1 and T2 already contain the textbook relation η = (−1)^{2s} = e^{2πis} [ESTABLISHED/KNOWN], and T2 constructs the exchange map σ_{M,M} for the symmetric case, identifying the single sign η = −1 with the treatise's half-turn e^{iπ} = −1 (and η = +1 with Calling); the ribbon identity η = θ_M is established. **RES.010 delta (precisely):** (a) R written as a *power of the half-turn*, R = (e^{iπ})^{2s} — the composite monodromy reading — which appears in neither RES.009 body nor T2; (b) the **unification of e, π, R as one family of logical scalars** (fixed point / trace / monodromy power) — not stated in any prior artifact; (c) the **treatise-anchored derivation** going beyond T2's symmetric-category construction: RES.009 itself documents the gap ("the calculus provides a phase for a single mark under rotation, but never constructs the exchange of two marks; the leap from 'the mark has a half-turn phase' to 'two marks anticommute' is asserted, not derived"), and T2 states "the treatise never constructs σ_{M,M}"; (d) the **quantized-case scoping**: (−1)^{2s} = ±1 requires 2s ∈ ℤ; the general anyon form is e^{2πis} — the equality chain in PROJECT-PLAN §1.2 is annotated accordingly (audit correction, this commit).
