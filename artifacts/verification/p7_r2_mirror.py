#!/usr/bin/env python3
"""P7: R2 mirror of the published deposit (R2-MIRROR-AFTER-PUBLISH-1).

Mirrors the 37 deposited files byte-identical from Zenodo to
qnfo-releases/2026/08/keyword-taxonomy-consilience/ via the Cloudflare REST
API (R2-OBJECTS-LISTING-SHAPE-1: result = plain list; PUT key percent-encoded;
verify via list etag=md5 + download-back byte comparison).
"""
import json
import os
import urllib.parse
...
PREFIX = "2026/08/keyword-taxonomy-consilience"
RECORD_ID = 22071421
...
# 1) deposit file list from published record
...
with open("artifacts/verification/p7_r2_mirror.json", "w", encoding="utf-8") as fh:
