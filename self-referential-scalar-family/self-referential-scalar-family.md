...
- **R2 (statistics = information, fermionic).** The squarefree restriction of the
  integers ($v_p \in \{0,1\}$ — the p-adic Pauli exclusion) yields $P(v_p = 1) =
...
formalism $H_p = \mathbb{E}[v_p]$ [1], the exchange-phase reading $R = (e^{i\pi})^{2s}$
...
2. **$e^{-\pi x^2}$ as the fixed point of the Fourier transform** (Adelic Shannon
   ...
   formula $\sum_n f(n) = \sum_n \hat{f}(n)$ holds *because* the Gaussian is this fixed
   point — source-channel equality at every place simultaneously.
3. **$e^{-\beta E}$ as the fixed point of the maximum-entropy variational principle**
...
$H_p(X) = \sum_x p(x)\, v_p(x) = \mathbb{E}[v_p(X)]$ and identifies the maximum-entropy
...
2. **The p-adic anyon embedding**: the p-adic anyon program realizes braiding phases at
   ...
   $\zeta_6 = e^{i\pi/3}$, $\zeta_8 = e^{i\pi/4}$ — the $(1/p^k)$-th roots of the
...
with the Planckian scattering time $\tau_\hbar \approx \hbar/k_B T$. The numerical
...
Planckian wall is the wall of the half-turn: $\tau_\hbar = \hbar/k_B T$ is the time in
...
$H_p^{\max} = 1/(p-1) = \langle n \rangle_{\mathrm{BE}}(\beta_p)$ is a physical
...
to resolve a p-adic digit is set by $k_B T_p$ with $T_p = 1/\ln p$ (in units where
$\beta = 1/k_B T$). The prime $p = 2$ (binary digit) has $T_2 = 1/\ln 2 \approx 1.4427$
— the temperature of the smallest prime, hence the highest of all p-adic temperatures;
higher primes are colder ($T_3 \approx 0.91$, $T_5 \approx 0.62$). [CONJECTURE — the
...
for a p-adically contractive channel $T$, $H_p(T(X)) \le H_p(X)$ — information cannot
...
$p = \{0.5, 0.25, 0.13, 0.06, 0.03, 0.02, 0.01\}$ over $v_2$, $H_p = 0.97$ before and
...
$\mathbf{I}(X) = (I_\infty, I_2, I_3, \ldots)$ of the measured observable. **QND is the
...
|---|---|---|---|
| Information theory | Gaussian $e^{-\pi x^2}$, Poisson summation | $\mathcal{F}[g] = g$ (Fourier self-duality) | $e^{-\pi}$, $\vartheta_3(0;i) = 1.0864\ldots$ |
| Statistics (quantum) | Exchange phase $R = e^{2\pi i s}$ | $(e^{i\pi})^{2s}$ — half-turn power | $e^{i\pi} = -1$ |
| Statistics (thermal, $\infty$) | Bose–Einstein / Fermi–Dirac | Max-entropy with fixed mean | $\beta = 1/k_B T$ |
| Statistics (thermal, $p$) | Geometric / squarefree valuation | Max-entropy with fixed mean valuation | $\beta_p = \ln p$ |
| Thermodynamics | Boltzmann factor $e^{-\beta E}$, Planckian bound | Max-entropy variational | $2\pi$ (circle trace), $\lambda_L \le 2\pi k_B T/\hbar$ |
| QND measurement | Information-conserving channel | Equality case of DPI ($H_p$ preserved) | $\mathbf{I}(X)$ conserved |
...
- **F1 (thermal p-adic scale).** If a physical process is exhibited whose energy cost
  per resolved p-adic digit scales other than $\sim k_B T_p = k_B/\ln p$ (or with a
  different prime ordering than $T_2 > T_3 > T_5 > \ldots$), the thermodynamic reading
  of $\beta_p$ is disconfirmed. The ordering $T_2 \approx 1.44 > T_3 \approx 0.91 >
...
1. **QND metrology with p-adic observables.** Any quantum sensor whose observable has
   ...
   $\mathbf{I}(X) = (I_\infty, I_2, I_3, \ldots)$ is the complete metrological error
   budget — a concrete specification for sensor calibration.
2. **p-adic noise models in quantum engineering.** The AUM channel with
   $\beta_p = \ln p$ gives a closed-form capacity $C_p = \log_p(1 + \mathrm{SNR}_p)$
   for noise whose valuation structure is prime-specific; the doubling
   $C_2 = 2C_\infty$ (verified for all SNR) is a design rule for binary-resolved
   systems.
3. **Energy benchmarking per prime digit.** The Joules-per-Solution metric [5] gains a
   ...
   $k_B T_p = k_B/\ln p$; a benchmark that reports energy per prime digit is directly
   comparable across architectures.
4. **Planckian design rule.** The MSS bound $\lambda_L \le 2\pi k_B T/\hbar$ with the
...
5. **Measurement-channel accounting.** The QND equality case gives a practical audit
   ...
   exactly $H_p(\text{before}) - H_p(\text{after})$, computable in situ.
...
[3] Quni-Gudzinas, R. B. (2026). The Exchange Phase as a Logical Scalar:
$R = e^{2\pi i s}$ from the Re-Entrant Calculus. 10.5281/zenodo.21964104 (concept
21941184).
...
[5] Quni-Gudzinas, R. B. (2026). The Joules-per-Solution Metric.
