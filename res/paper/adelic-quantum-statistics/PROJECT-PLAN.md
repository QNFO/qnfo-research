...
**Epistemic framing (user directive, 2026-08-27):** no physical particle is implied; no actual quantum particles are being claimed. The claims below are isomorphisms of mathematical structure: the lattice-restriction distributions and the golden occupation forms are the same mathematical object, and the physical labels (Bose–Einstein, Fermi–Dirac, anyonic) attach at the level of statistical distributions — not to any particle ontology. The register is therefore: "the occupation-number distributions of quantum statistics are the maximum-entropy distributions of a multiplicity-constrained lattice, up to isomorphism" — a structural correspondence carrying a precise mapping, a uniqueness argument, and testable consequences, never a statement about what particles are. This directive converts the Q9 dictionary-or-derivation guard into a standing rule: every claim is stated as an isomorphism plus its verification; nothing is asserted about the physical world beyond its statistical labels.
...
- **T3 (symplectic):** the symplectic (complex) structure J² = −1 of the large-N unitary limit (QNFO.RES.021) is selected in the Fermi case, where exclusion forces the skew-symmetric reversible generator, and is absent (symmetric/diffusive) in the Bose case. *(Precision, verified in verify_symplectic.py, 34/34: the complex structure is the sign-normalized generator — the discrete Hilbert transform, H² = −1 on mean-zero modes exactly at every N; the raw generator carries the Hilbert phase but the magnitude −sin²(2πk/N), and at fixed θ its J² stays bounded away from −1 as N grows; the Fermi-side specificity is exact — the Bose-side symmetric generator has real eigenvalues cos²(2πk/N) ≥ 0 and admits no complex structure.)*
...
- **F4 RE-SCOPE (2026-08-27, predecessor-note reconciliation; corrected to published-prior-art 2026-08-27):** the development note `_adelic-statistics-thermo-qnd-synthesis-2026-08-20.md` is the development artifact of **QNFO.RES.020** (identical title), and its verified results — R1/R2 clean in the 37-entry run; born_degeneracy FAIL flagged separately — are PUBLISHED in RES.020 v1.3.0 (10.5281/zenodo.22035210), whose abstract leads with them and whose premise register is exactly the isomorphism-only framing directed for this paper. R1 — the p-adic maximum-entropy distribution P(v_p = k) = (1−1/p)p^{−k} IS the Bose–Einstein occupation distribution at fugacity z = 1/p (β_p = ln p); R2 — the squarefree restriction yields P(v_p = 1) = 1/(p+1), the Fermi–Dirac occupation at the same β_p; R3 — QND as the equality case of the adelic data-processing inequality. The note's run also already computes anyon phases R = e^{2πis} for fractional s (s = 1/4 → i; s = 1/3 → e^{2πi/3}) — the phase side of F6, not the occupancy side. Those per-place identifications are therefore NOT part of RES.027's novel delta. The paper's delta is: (a) the publication-grade generalization of R1/R2 (arbitrary (β, μ, p) golden occupations — F1d — plus the global lattice identities — F1a/F1b/F1c); (b) T2, the γ = 1/N degeneracy derivation (the seed note's "final continuation"); (c) T3, the symplectic emergence as the large-N Fermi limit; (d) F5, the composite-parity/Möbius bridge; (e) F6, the anyonic/intermediate-statistics program; (f) the crosswalk + practitioner content. The predecessor note is cited as L2 input, not as prior art to be outrun. **Scope decision:** the predecessor note's R3 (QND = equality case of the adelic data-processing inequality) is cited-only and out-of-scope for this paper — it belongs to the QND/measurement tier, not the statistics tier; stating this explicitly prevents silent scope drift.
- **H-PARASTATS-INTERMEDIATE (predictive card, registered per red-team S3/S4; extended 2026-08-27 with anyon/Majorana targets):** the paper's predictive content — other lattice restrictions (partial exclusions) yield parastatistical/anyonic intermediates — is a hypothesis card, not an open-problem footnote. Dimensionality bridge (named, from `_26239072830.md`): d ≥ 3 → S_N exchange → exactly ±1; d = 2 → B_N → anyons; 2D BKT quasi-condensation; the proposed arithmetic analog: partial exclusion (bounded occupancy a_p ≤ m) ↔ intermediate statistics. **Known observable targets the analog must contact (F6 legs):** (F6a) abelian anyons — fractional exchange phases e^{iθ}, θ = π/m families (Laughlin quasiparticles, fractional quantum Hall, ν = 1/m fillings); (F6b) non-abelian anyons — Ising/Majorana (Majorana zero modes in semiconductor–superconductor hybrids, Kitaev chains; σ² = 1 braiding with matrix-valued action, 4π-periodic fractional Josephson effect) and Fibonacci (ν = 12/5, universal TQC); (F6c) parastatistics beyond 2D (Wang–Hazzard 2308.05203 — generalized exclusion / bounded-occupation machinery; their constructed models are 1D/2D, so the card reads "beyond 2D", not "in 3D"). Falsifier F6: if no intermediate occupation bound reproduces any known anyonic/parastatistical observable in the same max-entropy framework, the bridge is a dictionary without contact (Q9 discipline).
...
3. `verify_symplectic.py` — Fisher metric g + reversible generator L; J² = −1 check in the Fermi limit; symmetric-generator check in the Bose limit (F3). Executed 2026-08-27, 34/34: Fisher Hessian verified against the exact signed closed form (d²S = −δ/pᵢ − 1/p_N; the Fisher metric is its negative — the seed note's stated "g = d²S" carries a sign slip, the metric it uses throughout is the correct positive one — sign-convention note); L skew w.r.t. g; Fourier eigenvalues ±i sin(2πk/N) per shift convention; ω = gL antisymmetric; finite-N J² = −sin²(2πk/N) (not −1); Hilbert normalization H² = −1 on mean-zero modes at every N; Bose-side L_B symmetric with J_B² = cos² ≥ 0 (no complex structure); large-N pointwise honesty at fixed θ.
...
5. `verify_parastats.py` — the F6 math-side golden checks: bounded occupancy a_p ≤ m gives the single-mode partition function Z_m = (1 − z^{m+1}p^{−(m+1)β})/(1 − zp^{−β}) and the lattice identity Σ_{v_p ≤ m ∀p} n^{−s} = ζ(s)/ζ((m+1)s); m = 1 recovers the squarefree (Fermi) identity ζ(s)/ζ(2s) and m → ∞ recovers ζ(s) (Bose) — the interpolation family the anyon correspondence must contact. The anyon-phase correspondence itself remains an open problem, stated not verified.
