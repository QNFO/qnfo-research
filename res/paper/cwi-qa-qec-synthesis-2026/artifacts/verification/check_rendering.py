#!/usr/bin/env python3
"""RES.026 rendering gate — run BEFORE publish (CURRENCY-DOLLAR-ESCAPE-1,
...
Checks:
  ...
  4. raw U+FFFD scan in .md (informational; the PDF glyph gate is the
     decompressed-text-stream scan, per FFFD-RAW-FALSE-POSITIVE-1)
  5. reference-count parity: unique [n] markers vs rendered list count
...
def main():
    path = sys.argv[1] if len(sys.argv) > 1 else "cwi-qa-qec-synthesis-2026.md"
    ...
    if problems:
        ...
        for p in problems:
            print("  " + p)
        return 1
    print(f"RENDERING GATE: PASS ({n_refs} refs, no odd-$, no dup frontmatter, {fffd} U+FFFD)")
    return 0
