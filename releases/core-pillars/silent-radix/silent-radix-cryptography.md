...
- **Key sizes**: 512 decimal digits for $\lambda = 64$, scaling to 4096 digits for $\lambda = 128$.
...
| Security parameter $\lambda$ | Candidate bases | Brute-force cost | Status |
|:--------------------|:---------------|:-----------------|:-------|
...
digits for $\lambda = 128$, each attempt costs $\sim$ 4096 multi-precision
multiplications. At $\sim 10^9$ ops/sec, this is $\sim 4$ $\mu$s per attempt,
or $\sim 2^{64}$ seconds to exhaust a $\lambda = 128$ key space — well beyond any
practical horizon.
...
- **Large envelope size**: for $\lambda = 128$, the envelope $N$ requires ~4096
...
5. **Quantum attacks.** Grover's algorithm provides a quadratic speedup for
   ...
   secure at $\lambda = 256$ against quantum adversaries. Whether the
