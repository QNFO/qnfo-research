"""D1 backfill + DNSLink creation via wrangler CLI."""
...
for p in PAPERS:
    ...
    cmd = f'npx wrangler d1 execute living-paper --command="{sql}"'
    ...
    if ok and "ERROR" not in out:
        ...
        # Check if it was a "no rows matched" error (slug might not exist)
        if "no such" in out.lower() or "D1_ERROR" in out:
